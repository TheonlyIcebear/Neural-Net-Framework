...
class Adam:
    def __init__(self, learning_rate=0.01, momentum = 0.9, beta_constant = 0.99):
    ...
    def apply_gradient(self, values, gradient, descent_values):
        ...
        new_values = values - self.learning_rate * (new_gradient_momentum / (np.sqrt(new_squared_momentum + epsilon)))
        new_descent_values = [new_gradient_momentum, new_squared_momentum]

        return new_values, new_descent_values
                
class RMSProp:
    def __init__(self, learning_rate=0.01, beta_constant = 0.99):
        self.learning_rate = learning_rate
        self.adam_constant = beta_constant
    ...
    def apply_gradient(self, values, gradient, descent_values):
        ...
        new_values = values - self.learning_rate * (gradient / (np.sqrt(new_squared_momentum + epsilon)))
